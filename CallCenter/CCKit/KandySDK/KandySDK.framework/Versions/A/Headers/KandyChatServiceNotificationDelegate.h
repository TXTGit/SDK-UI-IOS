...
/**
 *  Received acknowledge for Chat message read
 *  You should mark the KandyReadAck as received, in order not to receive it again upon pulling events
 *
 *  @param ackData Acknowledges that the message you read was received by the other side
 */
-(void) onMessageRead:(KandyReadAck*)ackData;

/**
 *  Chat read notification state changed, this could happen if user changed read notification settings from his other device
 */
-(void) onReadNotificationStateChaged:(BOOL)isReadNotificationAllowed;
