...
#pragma mark - BlockContacts

/**
 * Get all blocked contacts
 * @param responseCallback - The callback that will be invoked
 *  @param kandyRecords - List of kandy records that are blocked (get them from the sever).
 *  @param error  out param, to be filled with error in case the server get to send blocked contacts

 */
-(void)getBlockedContactsListWithResponseCallback:(void(^)(NSError* error, NSArray *kandyRecords))responseCallback;

/**
 * UnBlock list of contacts - unblock call and chats from these contacts,
 * when you change the block status to unblock for each contact you will received in pendingEvents API
 * all the calls and chats from the time that you blocked this contact.
 * @param kandyRecords - list of contacts to block (kind of kandy record)
 * @param error  out param, to be filled with error in case the server unSucceded to unblock kandy records
 * @param responseCallback - The callback that will be invoked
 */
-(void)unblockContacts:(NSArray*)kandyRecords responseCallback:(void(^)(NSError* error))responseCallback;

/**
 * Block list of contacts - for block all communication from these contacts
 * @param kandyRecords - list of contacts to block (kind of kandy record)
 * @param error  out param, to be filled with error in case the server unSucceded to block kandy records
 * @param responseCallback - The callback that will be invoked
 */
-(void)blockContacts:(NSArray*)kandyRecords responseCallback:(void(^)(NSError* error))responseCallback;
