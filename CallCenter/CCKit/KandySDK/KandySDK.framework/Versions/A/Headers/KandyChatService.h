...
/**
 *  Mark chat messages as read by user.
 *
 *  @param messageUUIDs     The list of messages UUIDs
 *  @param responseCallback The callback, will return error if the requets was failed
 */
-(void) markMessagesAsRead:(NSArray<NSString*>*)messageUUIDs responseCallback:(void(^)(NSError* error))responseCallback;

/**
 *  Allow/disallow sending message read notifications. If you disable this feature you also will no longer recieve read notifications from other users
 *
 *  @param isReadNotificationAllowed a flag indicating if chat read notification is allowed
 *  @param responseCallback          a callback that will return error if action is failed, or nil on success
 */
-(void) setReadNotificationAllowed:(BOOL)isReadNotificationAllowed responseCallback:(void(^)(NSError* error))responseCallback;
