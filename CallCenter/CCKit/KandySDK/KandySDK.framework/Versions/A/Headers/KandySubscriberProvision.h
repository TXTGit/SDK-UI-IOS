 ...
 *  on failure returns an error and nil provisionedUserInfo
 */
-(void) getUserProvisionInfo:(NSString *)phoneNumber countryCode:(NSString *)countryCode responseCallback:(void (^)(NSError *error, KandyUserProvisionInfo *kandyUserProvisionInfo))responseCallback;

/**
 * retrieve the provision Data from the server
 * @param kandy record - record for which to retrieve the data
 * @param responseCallback A block of code to run when response returns, on success will return nil error and NSDictionary
 */
-(void) getUserProvisionData:(KandyRecord*)record responseCallback:(void (^)(NSError *error, NSDictionary *dictionary))responseCallback;
